  ...
  try {
    ...
    try {
      const result = await app.crawlUrl('https://www.aven.com/', {
        limit: 20,
        includePaths: ['/support', '/education', '/contact', '/reviews', '/app', '/about'],
        scrapeOptions: {
          formats: ["markdown"],
 ...
 */
export async function upsertChunksToPinecone(chunks: RAGChunk[]): Promise<void> {
  try {
    logger.action('upsertChunksToPinecone - Starting upsert process', { chunkCount: chunks.length });
    ...
    const namespace = pinecone.index(indexName, "https://aven-rag-jot4yxr.svc.aped-4627-b74a.pinecone.io").namespace("aven");
    await namespace.upsert(records);
    
    logger.action('upsertChunksToPinecone - Successfully upserted', { recordCount: records.length });
  } catch (error) {
 ...
 */
export async function upsertChunksToPineconeAction(chunks: RAGChunk[]): Promise<void> {
...
export async function querySimilarContent(query: string, topK: number = 5) {
  try {
    logger.action('querySimilarContent - Starting query', { query: query.substring(0, 100), topK });
...
export async function main() {
  ...
  try {
    ...
    logger.action("🧠 Step 3: Generating embeddings and storing in Pinecone...");
    await upsertChunksToPinecone(chunks);
